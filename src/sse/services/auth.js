...
import { resolveProviderId, FREE_PROVIDERS, isCustomEmbeddingProvider } from "@/shared/constants/providers.js";
...
export async function getProviderCredentials(provider, excludeConnectionIds = null, model = null, options = {}) {
  ...
  try {
    ...
    if (connections.length === 0) {
      // Custom embedding nodes (e.g. LM Studio) may run without an API key.
      // Fall back to a virtual connection using the node's baseUrl.
      if (isCustomEmbeddingProvider(providerId)) {
        ...
        if (node?.baseUrl) {
          ...
          log.debug("AUTH", `${provider} | no connection, using node baseUrl ${node.baseUrl}`);
          return {
            apiKey: "lm-studio",
            connectionName: node.name || "Local (no auth)",
            connectionId: "noauth",
            providerSpecificData: {
