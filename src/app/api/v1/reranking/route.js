...
function jsonError(status, message) {
  return new Response(
    JSON.stringify({ error: { message, type: "reranking_error" } }),
    {
      status,
      headers: {
...
export async function POST(request) {
  ...
  if (!query || typeof query !== "string")
    return jsonError(400, "Missing or invalid query");
  if (!Array.isArray(documents) || documents.length === 0)
    return jsonError(400, "documents must be a non-empty array");
  ...
  if (!cfg)
    return jsonError(400, `Provider '${provider}' does not support reranking`);

  // Always fetch credentials to get providerSpecificData (e.g. custom baseUrl for LM Studio)
  ...
  if (provider === "lm-studio")
    url = `${resolveLmStudioHost(credentials)}/v1/reranking`;

  // Build provider-specific request body
  const docs = documents
    .map((d) => (typeof d === "string" ? d : d?.text || ""))
    .filter(Boolean);
  const topN = Number.isFinite(Number(top_n))
    ? Number(top_n)
    : Number.isFinite(Number(top_k))
      ? Number(top_k)
      : undefined;
  let upstreamBody;
  if (cfg.format === "voyage") {
    upstreamBody = {
      ...
      model,
      ...(topN ? { top_k: topN } : {}),
      ...(return_documents !== undefined ? { return_documents } : {})
    };
  } else {
  ...
  const headers = {
    "Content-Type": "application/json",
    ...buildAuthHeader(cfg.authHeader, token)
  };
  ...
  const results = rawResults.map((r) => ({
    ...
    ...(r.document
      ? {
          document:
            typeof r.document === "string" ? { text: r.document } : r.document
        }
      : {})
  }));
  const normalized = {
    ...
    results,
    usage: data.usage || data.meta?.billed_units || undefined
  };
