...
import { resolveOllamaLocalHost, resolveLmStudioHost } from "open-sse/config/providers.js";
...
export async function GET(request, { params }) {
  try {
    ...
    if (connection.provider === "lm-studio") {
      const url = `${resolveLmStudioHost(connection)}/v1/models`;
      const response = await fetch(url, {
        method: "GET",
        headers: { "Content-Type": "application/json" },
      });
      if (!response.ok) {
        const errorText = await response.text();
        console.log(`Error fetching models from lm-studio:`, errorText);
        return NextResponse.json(
          { error: `Failed to fetch models: ${response.status}` },
          { status: response.status }
        );
