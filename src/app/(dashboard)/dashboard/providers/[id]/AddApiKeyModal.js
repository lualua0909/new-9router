...
export default function AddApiKeyModal({ isOpen, provider, providerName, isCompatible, isAnthropic, authType, authHint, website, proxyPools, onSave, onClose }) {
  ...
  const isLmStudio = provider === "lm-studio";
  const isLocalNoAuth = isOllamaLocal || isLmStudio;
  const localHostDefault = isLmStudio ? "http://127.0.0.1:1234" : "http://localhost:11434";
  const localHostLabel = isLmStudio ? "LM Studio Host URL" : "Ollama Host URL";
  const localDefaultName = isLmStudio ? "LM Studio" : "Ollama Local";
  ...
  const buildProviderSpecificData = () => {
    if (isLocalNoAuth && formData.ollamaHostUrl.trim()) {
  ...
  const handleSubmit = async () => {
    if (!provider) return;
    if (!isLocalNoAuth && !formData.apiKey) return;
    if (!isLocalNoAuth) {
    ...
    try {
      ...
      await onSave({
        name: formData.name || (isLocalNoAuth ? localDefaultName : ""),
    ...
    <Modal isOpen={isOpen} title={`Add ${providerName || provider} ${credentialLabel}`} onClose={onClose}>
      <div className="flex flex-col gap-4">
        <Input
          ...
          placeholder={isLocalNoAuth ? localDefaultName : "Production Key"}
        />
        {isLocalNoAuth && (
        ...
        {isLmStudio && (
          <p className="text-xs text-text-muted">
            Leave blank to use <code>http://127.0.0.1:1234</code>. Make sure the LM Studio local server is running (Developer tab → Start Server). For remote, enter full host URL (e.g. <code>http://192.168.1.10:1234</code>).
          </p>
        )}
        {validationResult && (
        ...
        <div className="flex gap-2">
          <Button onClick={handleSubmit} fullWidth disabled={saving || (!isLocalNoAuth && (!formData.name || !formData.apiKey)) || (isAzure && (!azureData.azureEndpoint || !azureData.deployment || !azureData.organization)) || (isCloudflareAi && !cloudflareData.accountId)}>
            {saving ? "Saving..." : "Save"}
          </Button>
