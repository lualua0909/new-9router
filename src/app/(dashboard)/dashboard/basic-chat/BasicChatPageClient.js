    ...
    async function loadData() {
      ...
      try {
        ...
        if (connections.length === 0) {
          if (!cancelled) {
            setProviderGroups([]);
            setLoadError("No providers are connected yet.");
          }
          return;
        }
        ...
        if (!cancelled) {
          ...
          if (normalized.length === 0) {
            setLoadError("Providers are connected, but no models could be loaded.");
          }
        }
      } catch (error) {
        if (!cancelled) {
          setLoadError(textValue(error?.message) || "Unable to load the provider/model list.");
          setProviderGroups([]);
        }
      } finally {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (error.name !== "AbortError") {
        ...
        setLoadError(errorText || "Unable to send the message.");
      }
    } finally {
      setIsSending(false);
      setStreamingMessageId("");
            ...
            {modelMenuOpen ? (
              <div className="absolute left-0 top-[calc(100%+10px)] z-30 w-[min(520px,calc(100vw-2rem))] overflow-hidden rounded-[20px] border border-white/10 bg-[#262626] shadow-2xl shadow-black/50">
                <div className="border-b border-white/10 px-4 py-3">
                  ...
                  <p className="text-sm text-white/75">Only models from connected providers are shown</p>
                </div>
