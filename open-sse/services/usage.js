...
async function getCursorUsage(accessToken) {
  if (!accessToken) {
    return { message: "Cursor access token not available. Please re-import the connection." };
  }

  return {
    message: "Cursor connected. Cursor quota details are not exposed by a supported usage API yet.",
  };
}
